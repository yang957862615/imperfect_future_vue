 ...
 *  @param tmpTime 时间戳
 * */
import {timestampToTime} from './timestamp_convertor.js'
export function timeDifference(tmpTime) {
  ...
  //将 yyyy-mm-dd H:m:s 进行正则匹配 (有bug 所以直接后台传时间戳过来)
  // let tmpTimeStamp = tmpTime ? Date.parse(tmpTime.replace(/-/gi, "/")) : new Date().getTime();
  // console.log('tmpTimeStamp:',tmpTimeStamp);
  let nowTime = new Date().getTime();//获取当前时间戳
  let tmpTimeDifference = nowTime - tmpTime;//计算当前与需要计算的时间的时间戳的差值
  if (tmpTimeDifference < 0) {        //时间超出，不能计算
    console.log("开始日期大于结束日期，计算失败！");
    return 0;
  }
  /**
  ...
  if (DifferebceMonth >= 1) {
    return timestampToTime(tmpTime); //大于一个月直接返回日期
    // return parseInt(DifferebceMonth) + "个月前";
